 ...
 *
 * A byte-for-byte copy of lib/site/accept.ts in the parent repo, below the
 * header comment. The duplication is forced by the deployment boundary, not
 * chosen: docs-site/Dockerfile copies only `docs-site/` and `docs/` into the
 * image, so the parent `lib/` does not exist at build or run time and cannot be
 * imported.
 *
 * tests/unit/docs-markdown.test.ts asserts the two implementations agree on a
 * table of Accept headers, so the copy cannot drift silently. Change both.
 */
