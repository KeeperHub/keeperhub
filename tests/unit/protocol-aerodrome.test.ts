...
describe("Aerodrome Protocol Definition", () => {
  ...
  it("describes scalar-array actions using the structured item editor", () => {
    ...
    expect(vote?.inputs.find((input) => input.name === "_poolVote")).toMatchObject(
      { label: "Pool Addresses", type: "address[]" }
    );
    expect(vote?.inputs.find((input) => input.name === "_weights")).toMatchObject(
      { label: "Vote Weights", type: "uint256[]" }
    );
    expect(
