...
Consecutive Write Contract nodes on one chain along one path are simulated as a sequence, each against the state the node before it produced, so an approve followed by a deposit is judged as it will actually run rather than warning on allowance because the approve has not landed. The nodes are taken in the order the edges connect them, not the order they were saved. The state is carried by `eth_simulateV1` where the chain's node offers it and by trace-derived `eth_call` overrides where it does not; if a node offers neither, the later nodes of the run report that they could not be simulated rather than answering against stale state.

A node that cannot join such a run is simulated on its own against current chain state: the first write after a Condition or any other fork, a write on a different chain from the one before it, a write whose inputs use runtime templates, and native and token transfers. A warning on such a node may depend on an earlier workflow step whose state change has not been applied, and says so. A warning on a node simulated within a sequence has had its earlier steps applied and says nothing of the kind.
