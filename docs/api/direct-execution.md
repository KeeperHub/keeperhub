...
A node that cannot join such a run is simulated on its own against current chain state: a write with more than one incoming connection, the second branch of a fan-out (the first branch continues the run), a write on a different chain from the one before it, a write whose inputs use runtime templates, a write that sends native value (only the single-call path can report a funding shortfall with the amount), the first write past the ten-node limit, and native and token transfers. A warning on such a node may depend on an earlier workflow step whose state change has not been applied, and says so. A warning on a node simulated within a sequence drops that hedge only when no other reachable write ran before the sequence started and every earlier call in the sequence landed. Every reachable node that changes chain state counts as such a write, including the ones this preflight does not simulate (Approve Token, Batch Write Contract, protocol writes and Tempo writes), so a run that begins after any of them, or after a transfer, keeps the hedge, because the sequence applied the run alone; and so does a node whose earlier call in the run reverted, because a reverted call applies no state.
...
The endpoint is protected by rate limiting, a maximum of 50 workflow nodes, a 15-second simulation deadline, and a ten-call bound on each simulated sequence. A preflight that overruns the deadline answers `503` with `error: "SIMULATION_TIMEOUT"` and no warnings at all, rather than a partial set; on the state-override mechanism a sequence costs three requests per call where a single node costs two, so a long run on a chain without `eth_simulateV1` is the case most likely to reach it.
