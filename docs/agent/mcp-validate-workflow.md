...
| `approve-without-allowance-check` | An Approve Token node, or a `write-contract` node whose method is `approve`, has no Check Allowance node upstream and no earlier approve of the same token to the same spender. Batch nodes are not covered: inside a batch the approve's owner is the Multicall3 contract, which the batch node's own description explains. `approve(address,uint256)` is also the ERC-721 signature, where the second argument is a token id and allowance semantics do not apply, so a `write-contract` approve whose declared ABI carries `ownerOf` or `setApprovalForAll` is skipped; the amount is read from the second argument only when the ABI declares `allowance`. A revoke (a second argument of `0`, or `amount: 0` on an Approve Token node) is never hinted on either node kind, and it suppresses a later approve of the same token and spender, because after it the allowance is known to be zero | When the approve is meant to run every time. An exact-amount approve is consumed by the spend that follows it, so it is not redundant; an unlimited approve (`max`) stays in place once it has landed, and later runs re-grant it. To skip the approve when the allowance already covers the amount, add a Check Allowance node and a Condition on its `allowance` output that routes around the approve; the check alone only reads |

### What "upstream" means for `missing-allowance-preflight` and `approve-without-allowance-check`
...
`approve-without-allowance-check` follows the same reachability rule and adds one more suppressor: an approve whose token and spender both match an approve that runs before it is not flagged, because the second approve then has a grant it can be compared against. Both addresses have to be literal on both nodes for that to apply. A spender given as a template reference, or a token given as a platform-listed id rather than a contract address, never suppresses the hint, so leaving information out never hides a warning. The message says which kind of approve it is looking at when the amount is a literal (`amount` on an Approve Token node; the second argument on a `write-contract` approve whose declared ABI is recognisably ERC-20): unlimited (`max` on an Approve Token node; MaxUint256 in decimal or hex as the raw second argument of a `write-contract` approve), or an exact amount; for a template amount, or an ABI that does not identify the standard, it says neither. Approve Token nodes on the shipped templates raise this hint: every one of them approves unlimited with no allowance read before it.
