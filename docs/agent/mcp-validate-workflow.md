...
| `approve-without-allowance-check` | An Approve Token node, or a `write-contract` / batch call whose method is `approve`, has no Check Allowance node upstream and no earlier approve of the same token to the same spender | Whenever the approve is meant to run every time: an exact-amount approve is consumed by the spend that follows it and is not redundant. Add a Check Allowance node only if the workflow should skip the approve when the allowance already covers the amount |
