...
| `approve-without-allowance-check` | An Approve Token node, or a `write-contract` / batch call whose method is `approve`, has no Check Allowance node upstream and no earlier approve of the same token to the same spender | When the approve is meant to run every time. An exact-amount approve is consumed by the spend that follows it, so it is not redundant; an unlimited approve (`max`) stays in place once it has landed, and later runs re-grant it. To skip the approve when the allowance already covers the amount, add a Check Allowance node and a Condition on its `allowance` output that routes around the approve; the check alone only reads |
